...
`;
        ...
        if (hours >= 19) {
            ...
            scenarioPrompt = `
# メッセージ生成の心構え
- クルーズ後の特別な時間を過ごせる飲食店を提案し、乗客の満足度を最大化するメッセージを生成します。
...
`;
            ...
            // 19時より前のシナリオ（クルーズへの期待感メッセージ）
            scenarioPrompt = `
# レストランの推薦は行わないでください。
# メッセージ生成の心構え
- 現在の時刻、天気、気温に基づき、クルーズへの期待感を高めるメッセージを生成してください。
- **天気や気温の直接的な記述は避け、クルーズで体験できる情景や感情を想起させる表現を使用してください。**
- メッセージには以下の要素を盛り込み、現在の時間帯に合わせたトーンで、独自のメッセージを生成してください。
  - **時間帯に合わせた挨拶や導入**（例：「素敵な一日を！」「午後のひととき、いかがお過ごしですか？」など、時間帯そのものを直接含めない）
  - **クルーズで楽しめる具体的な魅力**（例：夜景の美しさ、船内の快適さ、特別な体験など）
  - **乗客への期待を促す言葉**（例：ご期待ください、お楽しみください）
  - **必要に応じた行動のアドバイス**（例：暖かい服装でお越しください）
- **天候が良い場合（例: 晴れ）**: 夜景や景色、快適な船旅を想起させ、具体的な行動も促す表現で期待感を高めてください。
- **天候が優れない場合（例: 曇り、雨）**: 船内の温かさ、快適さ、特別なイベントなどを強調し、安全で楽しいクルーズ体験を想起させる表現で期待感を高めてください。
- **参考例（晴れの場合）**: 「素敵な一日を！今日は綺麗な夜景が待っています。暖かい服装で、記憶に残るクルーズをお楽しみください！」
- **参考例（曇りや雨の場合）**: 「午後のひととき、いかがお過ごしですか？船内では温かいお飲み物をご用意しております。快適なクルーズをお楽しみください。」
`;
        }
